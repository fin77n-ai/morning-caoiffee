...
- `src/curateDigest.js`: select at most six candidate articles across sources, merge same-day events, enrich them, then write a structured short digest. Maximum one lead, three briefs and one optional discovery; no fixed glossary or question.
- Public GitHub repository roots are read through the rendered README API before falling back to the HTML page; both use the same public-address, redirect, timeout and size checks. Preview `articleReadings.readFailures` records bounded reason codes for failed reads, including local non-public DNS results. Network protection is never bypassed for preview.
...
- Migration: apply legacy-only suppression before source and total candidate quotas so old entries cannot crowd out new ones. URLs only present in the legacy seven-day ledger stay suppressed until that window expires because their old facts are unknown. Events already tracked by V2 remain eligible for verified same-URL updates.
...
- Maximum three model calls: select, draft, then mandatory factual copy-edit. The last pass also fixes draft validation errors; its output must pass the same mechanical checks. The final review sees only the quoted passages that matched the source, plus draft claims and attribution metadata; unmatched quotes are removed from its evidence. It cannot browse the full article for unrelated details to justify a claim. Unknown/repeated selection IDs are removed before extraction and counted in preview metadata; an unreadable selection response or failed final review aborts. An unreviewed draft is never sent. Quiet days may stop after selection. Live samples showed that two calls plus quotation matching did not reliably preserve comparison scope, so the third call is reserved for review instead of an optional retry.
...
- Target 500-800 Chinese characters when evidence supports it, with a 150-250 character lead and 60-100 character short items. A lead permits up to six separately quoted facts, short items up to three. Keep sparse reports short; repository popularity is not a release date. The 900 Chinese-character and 3200 UTF-16 hard budgets remain. Model output ceiling is 6000 tokens to leave room for evidence JSON, not a target message length.
- Each final story supplies a concrete event definition. The final editor can split mistaken product-level groups, but cannot reuse a candidate across stories or repeat an identical event definition. The first fact is limited to 72 characters, remaining facts to 120 each. Each fact must make one claim supported by its own quote; this is an editorial constraint, not a deterministic proof of semantic truth.
- If a reviewed headline has 73-120 characters, a shorter reviewed fact from the same story can become the headline; the original sentence moves intact into the body. All facts and quotes remain unchanged and the entire story is revalidated. No extra model call or unreviewed rewrite is used; without a suitable short fact the story is still omitted. Preview metadata includes bounded `validationFailures` samples and `repairs`, separate from published stories.
- Enforce the final length budget by removing whole stories from the end (discovery, then briefs), never cutting sentences. Only the retained stories and URLs enter sent history.
- Only successful delivery records event and legacy URL history. A recovery record is written before sending, then updated with a minimal Telegram receipt and local-history status. Timeouts remain delivery_unknown and suppress a misleading failure message. Actions uploads any existing preview and recovery record even after history persistence fails. Corrupt event history fails visibly; sending and Git persistence cannot be made atomic. See docs/telegram-operations.md for recovery and fixed-evidence preview acceptance.
...
- Telegram keeps short headlines and source URLs visible and marks each nonempty detail paragraph as an `expandable_blockquote` entity. Entity offsets use UTF-16; preview and delivery recovery retain the same text/entities pair. The client controls how many opening lines remain visible. No callback service is needed. Plain text previews show the full content.
