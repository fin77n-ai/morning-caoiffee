...
- If the final pass still produces an invalid quote, omit that entire story and record the omission in preview metadata; promote the first remaining brief if the lead was removed. All remaining items still undergo the normal cross-story and length checks. Never truncate a quote to make a claim appear supported.
